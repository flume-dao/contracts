...
describe("Factory function unit test", function () {
  ...
  beforeEach("Setting up for Factory view function test", async function () {
    ...
    //       (
    //         await ethers.getContract("FlumeFactory")
    //       ).address,
  ...
  it("#0: unit test create a vault v2", async () => {
    ...
    expect(await vaultV2.name()).to.be.eq("Flume Vault USDC/WETH");
  ...
  it("#1: unit test get token name", async () => {
    ...
    ).to.be.eq("Flume Vault USDC/WETH");
  });
  ...
  it("#9: upgrade and Call", async () => {
    ...
    const deployResult = await deployments.deploy("Flume", {
      from: userAddr,
      args: [userAddr],
      libraries: {
    ...
    const newImplementation: Flume = (await ethers.getContractAt(
      "Flume",
