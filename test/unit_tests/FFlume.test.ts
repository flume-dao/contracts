...
import { MockFFlume } from "../../typechain";

const { ethers, deployments } = hre;

describe("Flume smart contract internal functions unit test", function () {
  this.timeout(0);

  let mockFFlume: MockFFlume;
  ...
  beforeEach("Setting up for Vault V2 functions unit test", async function () {
    ...
    mockFFlume = (await ethers.getContract(
      "MockFFlume"
    )) as MockFFlume;
  });
  ...
  it("#0: Test Subtract Admin Fees", async () => {
    ...
    const result = await mockFFlume.subtractAdminFees(
      rawFee0,
      rawFee1,
      managerFeeBPS
    );
  ...
  it("#1: Test Compute Mint Amounts, when current1 is Zero", async () => {
    ...
    const result = await mockFFlume.computeMintAmounts(
      current0,
      current1,
      totalSupply,
      amount0Max,
  ...
  it("#4: Test mulDiv edge cases", async () => {
    ...
    const result1 = await mockFFlume.checkMulDiv(
      oneEth.mul(10),
      oneEth.sub(1),
      oneEth
    );
    ...
    const result2 = await mockFFlume.checkMulDiv(
      fivek,
      oneEth.sub(1),
      oneEth
    );
    ...
    const result3 = await mockFFlume.checkMulDiv(
