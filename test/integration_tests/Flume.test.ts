...
describe("Flume integration test!!!", async function () {
  this.timeout(0);

  let user: Signer;
  let user2: Signer;
  let userAddr: string;
  let arrakisV2Factory: FlumeFactory;
  let vaultV2: Flume;
  let pool: IUniswapV3Pool;
  let uniswapV3Factory: IUniswapV3Factory;
  let uniswapV3Pool: IUniswapV3Pool;
  let arrakisV2Resolver: FlumeResolver;
  ...
  beforeEach("Setting up for V2 functions integration test", async function () {
    ...
    // vaultV2 = (await ethers.getContract("Flume", user)) as Flume;
  ...
  it("#0: Deposit token and Mint Flume tokens ", async () => {
  ...
  it("#1: Burn Minted Flume tokens", async () => {
  ...
  it("#2: Rebalance after mint and burn of Flume tokens", async () => {
    // #region mint arrakis token by Lp.
  ...
  it("#3: Rebalance without swap after mint and burn of Flume tokens", async () => {
    // #region mint arrakis token by Lp.
