...
import {FlumeFactoryStorage} from "./abstract/FlumeFactoryStorage.sol";
import {InitializePayload} from "./structs/SFlume.sol";
import {
    EnumerableSet
} from "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import {_getTokenOrder, _append} from "./functions/FFlumeFactory.sol";

/// @title FlumeFactory factory for creating vault instances.
contract FlumeFactory is FlumeFactoryStorage {
    using EnumerableSet for EnumerableSet.AddressSet;

    constructor(IFlumeBeacon arrakisV2Beacon_)
        FlumeFactoryStorage(arrakisV2Beacon_)
    {} // solhint-disable-line no-empty-blocks

    /// @notice Deploys an instance of Vault using BeaconProxy or TransparentProxy.
    /// @param params_ contains all data needed to create an instance of Flume vault.
    /// @param isBeacon_ boolean, if true the instance will be BeaconProxy or TransparentProxy.
    /// @return vault the address of the Flume vault instance created.
    ...
    /// @notice get Flume standard token name for two corresponding tokens.
        ...
        returns (string memory)
    {
        ...
        return _append("Flume Vault ", symbol0, "/", symbol1);
    }
